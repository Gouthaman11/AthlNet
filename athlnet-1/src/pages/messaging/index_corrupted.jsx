...
import { saf      if (error.code === 'permission-denied' || error.message?.includes('permission-denied')) {
        errorMessage = 'Permission denied. Setting up user profiles for messaging...';
        safeLog.error('🔐 Firebase permission issue detected');
        
        // Try to auto-fix by ensuring user documents exist and retrying
        try {
          safeLog.log('Attempting to fix permission issue...');
          ...
          const messageId = await sendMessageToUser(user.uid, userId, content);
          safeLog.log('✅ Message sent successfully after fixing permissions');
          setShowNewMessageModal(false);
          return; // Success!
        } catch (retryError) {
          safeLog.error('❌ Auto-fix attempt failed:', retryError.message);
          errorMessage = 'Permission issue could not be automatically resolved. Please refresh the page and try again.';
        }g } from '../../utils/safeLogging';
...
const MessagingPage = () => {
  ...
  const handleSendMessage = async (content, mediaUrl = null) => {
    ...
    try {
      ...
      const messageId = await sendMessageToUser(
        user.uid,
  ...
  const handleNewMessage = async (userId, content) => {
    ...
    try {
      ...
      if (error.code === 'permission-denied' || error.message?.includes('permission-denied')) {
        errorMessage = 'Permission denied. If you just created your account, try refreshing the page and logging in again.';
        safeLog.error('� Firebase permission issue detected');
      } else if (error.message?.includes('Missing or insufficient permissions')) {
        errorMessage = 'Account permission issue. Please refresh the page and try again. If the problem persists, try logging out and back in.';
      } else if (error.message?.includes('not found') || error.message?.includes('search')) {
        errorMessage = 'Unable to find the target user. They may need to complete their profile setup.';
        safeLog.error('🚨 User profile issue detected');
      } else if (error.message?.includes('profile not found')) {
        errorMessage = 'This user may have an incomplete profile. Trying alternative messaging method...';
        ...
        try {
          await sendMessageWithBasicValidation(user.uid, userId, content);
